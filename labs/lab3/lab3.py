...
class Player:
    """
    Abstract class player for the number game
    ===Attributes===
...
class RandomPlayer(Player):
    """
    Player for the Number game who chooses random moves
    """
    ...
    def move(self,
             ...
             goal: int) -> int:
        """
        Return the move that the random player makes based on the <current>,
...
class UserPlayer(Player):
    """
    player for the number game where the user supplies the move
    """
    ...
    def move(self,
        ...
        """
        while True:
            movement = input(f'Enter a move between '
                             f'{min_step} and {max_step}: ')
            if not movement.isnumeric() or (int(movement) < min_step or
                                            int(movement) > max_step):
                print('Invalid Move')
            else:
                return int(movement)
...
class StrategicPlayer(Player):
    """
    Player that make moves strategically
    """
    ...
    def move(self,
             ...
             goal: int) -> int:
        """
        Make a move strategically based on the <current>, <min_step>, <max_step>
        and <goal>
        """
        if max_step + current >= goal:
            return max_step
        else:
            return min_step
...
def make_player(generic_name: str) -> Player:
    ...
    invalid = True
    while True:
        player_type = input(f'Which type of player is '
                            f'{generic_name}?[Random, User, Strategic]: ')

        if player_type.lower() == 'random':
            return RandomPlayer(name)
        elif player_type.lower() == 'user':
            return UserPlayer(name)
        elif player_type.lower() == 'strategic':
